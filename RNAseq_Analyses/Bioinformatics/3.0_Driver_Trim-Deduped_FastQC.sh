...
#SBATCH --job-name=ZF_TrimDriver3_dedup              #-- For convenience, give your job a name
...
#SBATCH --output=Trim_DedupMap%A-%a.out   #-- Changes the output to correspond to each subjob
#SBATCH --error=Trim_DedupMap%A-%a.err    #-- Changes the error to correspond to each subjob
#SBATCH --nodes=1                               #-- Specify the number of nodes and cores you want to use. Standard nodes 20 core
#SBATCH --partition=general			#general
#SBATCH --mem=20GB
...
./3.1_TRIMMO.sh   "$sample_id"

############# Run FASTQC (Again on the clean dedup paired data)
./3.2_FASTQC-CL_Dedup_paired.sh  "$sample_id"
